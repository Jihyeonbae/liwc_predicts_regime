...
trainN<-trainN %>%
  dplyr::rename(function_liwc = `function`) %>%  # avoid reserved word otherwise it will throw an error
  dplyr::select(-text)
...
imp_df1 <- importance(bag.democracy1, type = 1, scale = TRUE) |>
  as.data.frame() |>
  rownames_to_column("feature") |>
  dplyr::arrange(desc(abs(MeanDecreaseAccuracy)))

# CSV for  re-use
write_csv(imp_df1, "export/tables/importance_trial1.csv")

# LaTeX table for Overleaf (top 20)
imp_tab1 <- imp_df1 |>
  head(20) |>
...
save_kable(imp_tab1, "export/tables/importance_trial1.tex")

# and then the plot

png("export/figs/varImp_trial1.png", width = 2000, height = 1400, res = 300)
varImpPlot(bag.democracy1, type = 1, scale = TRUE, sort = TRUE,
           n.var = 25, main = "Democracy vs. Non-democracy", pch = 16)
...
imp_explainer_top20 <- importance_frame %>%
  as.data.frame() %>%
  # keep useful columns; add BH-adjusted p-values
  dplyr::transmute(
    variable,
    accuracy_decrease,
    gini_decrease,
    no_of_nodes,
    p_value = p_value,                          # from randomForestExplainer
    p_adj_BH = p.adjust(p_value, method = "BH") # multiple-testing control
  ) %>%
  dplyr::arrange(desc(abs(accuracy_decrease))) %>%
  slice_head(n = 20)

tab_expl <- imp_explainer_top20 %>%
  mutate(
    accuracy_decrease = round(accuracy_decrease, 3),
    gini_decrease     = round(gini_decrease, 3),
    p_value           = signif(p_value, 3),
    p_adj_BH          = signif(p_adj_BH, 3)
  ) %>%
  kbl(format = "latex", booktabs = TRUE,
      caption = "RandomForestExplainer importance with p-values (Top 20, Trial 1)",
      col.names = c("Feature","Accuracy ↓","Gini ↓","Nodes","p","p (BH)")) %>%
  kable_styling(latex_options = c("hold_position","scale_down"))

save_kable(tab_expl, "export/tables/importance_explainer_trial1.tex")
...
cm_df <- as.data.frame(RT.evlau1$table)

dir.create("export/tables", recursive = TRUE, showWarnings = FALSE)
readr::write_csv(cm_df, "export/tables/confusion_trial1.csv")
...
save_kable(cm_tab, "export/tables/confusion_trial1.tex")

RT.pred.roc1 <- predict(bag.democracy1, newdata=testN, type="prob")
auc_val <- pROC::auc(roc_RT.tree1) 
roc_RT.tree1 <- roc(as.factor(testN$dd_democracy), RT.pred.roc1[,"1"])
par(mfrow=c(1,1))

png("export/figs/roc_randomforest_trial1.png", width=2000, height=1400, res=300)
par(mfrow=c(1,1))
plot(roc_RT.tree1,
     main = "ROC curve for Random Forest",
     col = "blue", lwd = 2, legacy.axes = FALSE,
     print.auc = TRUE,                 # add AUC text
     print.auc.x = 0.6,                # x position (Specificity axis, 0=right, 1=left)
     print.auc.y = 0.98,               # y position (Sensitivity axis; 1 is top)
     print.auc.cex = 1.2,              # text size
     print.auc.col = "black")
dev.off()
